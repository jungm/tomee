...
public class LightweightWebAppBuilder implements WebAppBuilder {
    ...
    public void deployWebApps(final AppInfo appInfo, final ClassLoader appClassLoader) throws Exception {
        ...
        for (final WebAppInfo webAppInfo : appInfo.webApps) {
            ...
            bindings.putAll(new JndiEncBuilder(webAppInfo.jndiEnc, injections, "Bean", webAppInfo.moduleId, null, webAppInfo.uniqueId, classLoader, appInfo.properties).buildBindings(JndiEncBuilder.JndiScope.comp));

            final WebContext webContext = new WebContext(appContext);
            webContext.setBindings(bindings);
            webContext.getBindings().putAll(new JndiEncBuilder(webAppInfo.jndiEnc, injections, "Bean", webAppInfo.moduleId, null, webAppInfo.uniqueId, classLoader, appInfo.properties).buildBindings(JndiEncBuilder.JndiScope.comp));
